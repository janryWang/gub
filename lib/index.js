...
var traverseFile = async function traverseFile(dir, traverse) {
  var files = await _fsExtra.default.readdir(dir);

  for (var i = 0; i < files.length; i++) {
    try {
      var file_path = _path.default.join(dir, files[i]);

      var status = await _fsExtra.default.stat(file_path);

      if (status) {
        if (status.isFile()) {
          var content = await _fsExtra.default.readFile(file_path, "utf-8");

          if (typeof traverse === "function") {
            var newContent = traverse(content);
            if (newContent !== undefined) await _fsExtra.default.writeFile(file_path, newContent);
          }
        } else {
...
var cloneRepo = async function cloneRepo(repos, branch, dir) {
  if (dir === void 0) {
    dir = process.cwd();
  }
  ...
  try {
    ...
    await _execa.default.shell("git clone " + (branch ? "-b " + branch : "") + " " + repos + " " + tmp_path);
    ...
    await traverseFile(dir, function (target) {
...
_commander.default.command("init [repos] [dir]").option("-b, --branch <branch>", "Init with git branch").action(async function (repos, dir, options) {
  try {
    ...
    if (no_repos) {
      await (0, _alias.addAlias)(repos, (0, _parseGithubUrl.default)(repos).name);
    }
  } catch (e) {
    if (e) {
